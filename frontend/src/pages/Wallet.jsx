  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-12 relative z-20">
        {loading ? (
          <div className="bg-white rounded-[2.5rem] shadow-xl p-20 flex flex-col items-center justify-center gap-4 text-center border border-slate-100">
             <Loader2 size={48} className="text-indigo-600 animate-spin" />
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Encrypted Session Sync...</p>
          </div>
        ) : (
          <>
            {/* Main Wallet Card */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
              <div className="lg:col-span-2 bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-slate-900/30">
                ...
                <div className="relative z-10">
                  <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] mb-2">Available Credits</p>
              ...
              <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col justify-between">
                 <div>
                    <div className="flex items-center gap-3 mb-6">
                       ...
                       <h3 className="font-black text-slate-900 uppercase tracking-widest text-[10px]">Loyalty Points</h3>
                    </div>
                    <p className="text-4xl font-black text-slate-900 tracking-tighter mb-2">{wallet?.loyaltyPoints || 0}</p>
                 ...
                 <button 
                   ...
                   className="w-full py-4 bg-amber-50 hover:bg-amber-100 disabled:opacity-30 text-amber-600 rounded-2xl transition-all font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2"
                 >
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
              <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-white sticky top-0 z-10">
                ...
                <div className="flex items-center gap-2">
                   ...
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Verified Transactions Only</p>
                </div>
              </div>
              ...
              <div className="divide-y divide-slate-50">
                {transactions.length === 0 ? (
                  <div className="p-20 text-center flex flex-col items-center gap-4 opacity-30">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-slate-400">No history found</p>
                  </div>
                ) : (
                  transactions.map((tx, index) => (
                    <motion.div
                      ...
                      className="p-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:bg-slate-50/50 transition-all border-l-4 border-transparent hover:border-indigo-600"
                    >
                      <div className="flex items-center gap-6">
                        <div
                          className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border-2 ${
                            tx.type === "credit"
                              ? "bg-emerald-50 border-emerald-100 text-emerald-600 shadow-lg shadow-emerald-600/5"
                              : "bg-rose-50 border-rose-100 text-rose-600 shadow-lg shadow-rose-600/5"
                          }`}
                        >
                          {tx.type === "credit" ? (
                        ...
                        <div>
                          ...
                          <h4 className="text-lg font-black text-slate-900 tracking-tight truncate max-w-xs sm:max-w-md">
                            {tx.description}
                          </h4>
                        </div>
                      </div>
                      <div
                        className={`text-2xl font-black tracking-tighter sm:text-right ${tx.type === "credit" ? "text-emerald-600" : "text-slate-900"}`}
                      >
