    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-32 pb-24 px-4 overflow-hidden relative transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto text-center relative z-10">
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-6 py-2 bg-indigo-600/10 border border-indigo-600/20 text-indigo-600 dark:text-indigo-400 rounded-full text-xs font-black uppercase tracking-widest mb-8 shadow-sm"
        >
        ...
        >
          Choose the <span className="text-indigo-600 dark:text-indigo-400">Perfect Plan</span>
        </motion.h1>
        ...
        <motion.p
          ...
          className="text-lg text-slate-500 dark:text-slate-400 max-w-2xl mx-auto mb-20 font-semibold leading-relaxed"
        >
          ...
          {plans.map((plan, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`flex flex-col relative bg-white dark:bg-slate-900 rounded-[3rem] p-10 border transition-all duration-500 group ${
                  isFeatured 
                    ? "border-indigo-600 shadow-2xl shadow-indigo-100 dark:shadow-none ring-4 ring-indigo-50 dark:ring-indigo-900/20 lg:-mt-4 lg:mb-4" 
                    : "border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none hover:shadow-2xl"
                }`}
              >
                {isFeatured && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-indigo-600 text-white px-6 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase flex items-center gap-2 shadow-lg ring-4 ring-white dark:ring-slate-900">
                ...
                <div className="mb-10 text-left">
                  <div className="flex items-center gap-3 mb-6">
                    <div className={`p-3 rounded-2xl ${isFeatured ? "bg-indigo-600 text-white" : "bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 group-hover:bg-slate-900 dark:group-hover:bg-indigo-600 group-hover:text-white transition-colors"}`}>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 font-bold text-sm leading-relaxed min-h-[48px]">
                    {plan.description}
                  </p>
                </div>

                <div className="flex-1 space-y-5 mb-12">
                  <div className="h-[1px] w-full bg-slate-50 dark:bg-slate-800 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/30 transition-colors" />
                  {plan.features.map((feature, i) => (
                    <div key={i} className="flex items-start gap-4 text-left">
                      <div className={`mt-1 p-0.5 rounded-full ${isFeatured ? "bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400" : "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500 dark:text-emerald-400"}`}>
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-base flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg group/btn ${
                    plan.price === 0 
                      ? "bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-600 cursor-default shadow-none" 
                      : isFeatured
                        ? "bg-indigo-600 text-white hover:bg-slate-900 dark:hover:bg-indigo-500 hover:shadow-indigo-200 dark:hover:shadow-none"
                        : "bg-slate-900 dark:bg-slate-800 text-white hover:bg-indigo-600 hover:shadow-slate-200 dark:hover:shadow-none"
                  }`}
                >
