...
const AdminServicesTab = ({ services = [], handleDeleteService, handleModerateService }) => {
  ...
  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="p-10 border-b border-slate-50 bg-slate-50/30 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
        <div>
        ...
        <div className="relative w-full lg:w-96 group">
          ...
          <input
            ...
            className="w-full bg-white border-2 border-slate-100 rounded-2xl py-4 pl-14 pr-6 text-[11px] font-black text-slate-700 uppercase tracking-widest outline-none focus:border-indigo-600 focus:shadow-2xl focus:shadow-indigo-500/10 transition-all"
          />
        </div>
      </div>
            ...
            <tr className="bg-slate-50/80 backdrop-blur-md">
              {["Asset Identity", "Taxonomy", "Custodian", "Status", "Management"].map(
                (h) => (
                  <th
                    key={h}
                    className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] border-b border-slate-100"
                  >
                    {h}
                  </th>
                ),
              ...
              <tr key={s._id} className="group hover:bg-slate-50/50 transition-all duration-300">
                <td className="px-10 py-8">
                  <div className="flex items-center gap-6">
                    <div className="w-16 h-16 rounded-2xl bg-slate-100 border border-slate-200 overflow-hidden shadow-sm group-hover:scale-110 group-hover:shadow-xl transition-all duration-500 relative">
                       {s.images?.[0]?.url ? (
                         <img 
                           src={s.images[0].url} 
                           alt={s.title}
                    ...
                    <div>
                      <div className="font-black text-slate-900 text-lg tracking-tight leading-none mb-2 group-hover:text-indigo-600 transition-colors">
                        {s.title}
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5 text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-1 rounded-lg border border-amber-100/50">
                          <Star size={10} className="fill-amber-600" /> {s.rating.toFixed(1)}
                        </div>
                ...
                <td className="px-10 py-8">
                  <span className="inline-block px-4 py-1.5 bg-slate-900 text-white rounded-xl font-black text-[9px] uppercase tracking-[0.2em] shadow-lg shadow-slate-900/10">
                ...
                <td className="px-10 py-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Verified Partner</span>
                    </div>
                  </div>
                </td>
                <td className="px-10 py-8">
                  <div className="flex flex-col">
                    ...
                    <div className="text-xl font-black text-slate-950 tracking-tighter leading-none mb-1">
