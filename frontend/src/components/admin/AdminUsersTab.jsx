      ...
      <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="relative w-full md:w-80 group">
          ...
          <input
            ...
            className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-3.5 pl-12 pr-4 text-sm font-bold text-slate-700 outline-none focus:border-indigo-600 focus:bg-white transition-all shadow-inner"
          />
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredUsers.map((u) => (
              <tr key={u._id} className="group hover:bg-indigo-50/20 transition-colors">
                <td className="px-8 py-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-center justify-center text-lg font-black text-indigo-600 group-hover:scale-105 transition-transform">
                ...
                <td className="px-8 py-6">
                  {u.isBlocked ? (
                    <div className="flex items-center gap-2.5 text-xs font-black text-rose-600 uppercase tracking-widest bg-rose-50 w-fit px-3 py-1.5 rounded-lg border border-rose-100 italic">
                      <ShieldAlert size={14} className="fill-rose-600 text-white" />
                      Blocked
                    </div>
                  ) : (
                    <div className="flex items-center gap-2.5 text-xs font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 w-fit px-3 py-1.5 rounded-lg border border-emerald-100">
                      <CheckCircle size={14} className="fill-emerald-600 text-white" />
                      Active
                    </div>
                  )}
                  {u.isEmailVerified && (
                     <div className="mt-1 text-[0.6rem] font-bold text-slate-400 uppercase flex items-center gap-1">
