            ...
            {withdrawals.map((w) => (
              <tr key={w._id} className="group hover:bg-slate-50/50 transition-colors">
                <td className="px-8 py-6">
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-500 bg-slate-100/50 w-fit px-2.5 py-1 rounded-lg">
                ...
                <td className="px-8 py-6">
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-2 text-[0.65rem] font-black text-slate-700 bg-indigo-50 w-fit px-2 py-0.5 rounded border border-indigo-100 uppercase tracking-widest">
                      {w.paymentMethod === "bank_transfer" ? <Landmark size={12} /> : <CreditCard size={12} />}
                      {w.paymentMethod.replace("_", " ")}
                    </div>
                    {w.paymentMethod === "bank_transfer" && (
                      <div className="text-[0.7rem] font-bold text-slate-400 leading-tight">
                ...
                <td className="px-8 py-6">
                  <Badge 
                    ...
                    className="px-3 py-1 uppercase tracking-widest text-[0.6rem]"
                  >
                    {w.status}
                  </Badge>
                </td>
        ...
        {rejectingId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-white rounded-[2.5rem] w-full max-w-lg p-10 shadow-2xl border border-slate-100"
            >
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-900 tracking-tight">Reject Payout</h3>
                    <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-0.5">Please provide a reason</p>
                  </div>
                </div>
                <button 
                  onClick={() => setRejectingId(null)}
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl p-5 text-sm font-bold min-h-[120px] outline-none focus:border-rose-100 focus:bg-white transition-all ring-0 shadow-inner"
                  />
                </div>
